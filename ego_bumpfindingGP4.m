...
for g = 1: iter_size
    ...
    if nboxes > 2 && mod(g, 5) == 0  % local search frequency
        ...
        for ii = 1: nboxes -1
            ...
            if size(localx, 1)>= kb_initsize                  % minimum number of local search point
                if inboxcheck(minx, upper_bound, lower_bound) % fmin included
                    localsurrogate = true;
                    kbs = size(localx, 1);
                    % fprintf('local search size %d', kbs);
                    break;
                end
            end
        end
...
[match_xl, n_fev, flag] = post_infillsaveprocess(xu, train_xl, train_fl, train_fc, 'lladp_bh4', seed, prob, init_size);
