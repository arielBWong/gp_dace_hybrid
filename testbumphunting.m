function testbumphunting()
prob = Zakharov(1, 1);
% prob = Shekel(1, 1);

tstx = linspace(prob.xl_bl, prob.xl_bu, 10);
tstx = tstx';
tsty = prob.evaluate_l([], tstx);
plot(tstx, log(tsty)); hold on;

aa = primparams();
[prim, ~] = primbuild(tstx, -tsty, aa);
...
for i = 1:2
    ...
    radius = (ub - lb)/2;
    middle  = lb + radius;
    xn = linspace(lb, ub, 100);
    yn = sqrt(radius^2 - (xn - middle).^2);
    plot(xn, yn); hold on;
